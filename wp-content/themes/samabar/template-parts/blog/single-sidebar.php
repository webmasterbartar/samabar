<?php
/**
 * Single post sidebar with table of contents.
 *
 * @package Samabar
 */
...
$active_cat = samabar_get_primary_category();
$active_slug = $active_cat ? $active_cat->slug : '';
...
?>
<aside class="blog-sidebar blog-sidebar--single" aria-label="<?php esc_attr_e( 'نوار کناری مقاله', 'samabar' ); ?>">
	<div class="blog-widget blog-widget--toc" id="blog-toc-widget" hidden>
		<h2 class="blog-widget__title text-headline-md"><?php esc_html_e( 'فهرست مطالب', 'samabar' ); ?></h2>
		<nav class="blog-toc" id="blog-toc" aria-label="<?php esc_attr_e( 'فهرست مطالب', 'samabar' ); ?>"></nav>
	</div>
	...
	<div class="blog-widget">
		...
		<ul class="blog-widget__list">
			<li>
				<a class="blog-widget__link<?php echo '' === $active_slug ? ' is-active' : ''; ?>" href="<?php echo esc_url( $blog_url ); ?>">
					<?php esc_html_e( 'همه مقالات', 'samabar' ); ?>
				</a>
			</li>
			<?php foreach ( $categories as $term ) : ?>
				<li>
					<a class="blog-widget__link<?php echo $active_slug === $term->slug ? ' is-active' : ''; ?>" href="<?php echo esc_url( samabar_get_blog_filter_url( array( 'category' => $term->slug ) ) ); ?>">
